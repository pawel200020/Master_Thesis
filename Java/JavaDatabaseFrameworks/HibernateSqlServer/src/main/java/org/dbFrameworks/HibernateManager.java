...
public class HibernateManager implements IRelationalFrameworkManager {
        ...
        try (ProgressBar pb = new ProgressBar("SearchRecordsWhichDoesNotHaveConnection", samplesQuantity)) {
            for (var i = 0; i < samplesQuantity; i++) {
                if (i % 2 == 0) {
                    stopwatch.start();
                    em.getTransaction().begin();
                    var q = em.createQuery("SELECT p FROM Positions p WHERE p.Employee = null", Positions.class);
                    var result = q.getResultList().toArray();
                    ;
                    em.getTransaction().commit();
                    testResult.AddMeasure(stopwatch.stop().elapsed(TimeUnit.MILLISECONDS));
                    ...
                    stopwatch.start();
                    em.getTransaction().begin();
                    var q = em.createQuery("SELECT p FROM Stores p WHERE p.Orders = null", Stores.class);
                    var result = q.getResultList().toArray();
                    em.getTransaction().commit();
                    testResult.AddMeasure(stopwatch.stop().elapsed(TimeUnit.MILLISECONDS));
                    pb.step();
    ...
    public TestResult SearchWithSubQuery(int samplesQuantity) {
        ...
        try (ProgressBar pb = new ProgressBar("SearchWithSubQuery", samplesQuantity)) {
            for (var i = 0; i < samplesQuantity; i++) {
                var countryToFind = ThreadLocalRandom.current().nextInt(0, countries.size());
                stopwatch.start();
                em.getTransaction().begin();
                var q = em.createQuery("SELECT o FROM Orders o WHERE o.client.Country in (SELECT c FROM Clients C where c.Country =:countryName)", Orders.class);
                q.setParameter("countryName", countryToFind);
                var result = q.getResultList().toArray();
                em.getTransaction().commit();
                testResult.AddMeasure(stopwatch.stop().elapsed(TimeUnit.MILLISECONDS));
                pb.step();
