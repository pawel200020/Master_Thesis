...
{
    public class EntityFrameworkSqliteManager(FrameworkPerformanceSqlLiteContext context) : IRelationalFrameworkManager
    {
        private readonly FrameworkPerformanceSqlLiteContext _context = context ?? throw new ArgumentNullException(nameof(context));
        ...
        public TestResult SearchTwoRelatedTables(int samplesQuantity)
        {
            ...
            using (var progress = new ProgressBar())
            {
                for (int i = 0; i < samplesQuantity; i++)
                {
                    ...
                    var _ = _context.Employees.Where(x => x.Position.PositionName == positionToFind!.PositionName).ToArray();
